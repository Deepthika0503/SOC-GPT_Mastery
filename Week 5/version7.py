...
# scaling up
# Run this only if you have a GPU, which you don't have :)
...
n_head = 6
n_layer = 6
dropout = 0.2  # will take 20% of the network
...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        )  # Since this is not a parameter we have to define it this way
...
class FeedForward(nn.Module):
    ...
    # Now having gathered all the communication data, the tokens have to this on themselves independently
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            ...
            nn.Dropout(
                dropout
            ),  # added right before the residual connection or right before the connection back to the original network
        )
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.sa(self.ln1(x))  # residual connections or skip connections
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.lm_head = nn.Linear(
            n_embd, vocab_size
        )  # linear layer to get logits from the token embeddings
    ...
    def forward(self, indx, targets=None):
        ...
        tok_emb = self.token_embedding_table(
            indx
        )  # plucks out a row from the embedding table corresponding to the indx, it will be a vocab_size tensor
        ...
        if targets == None:
            ...
            loss = F.cross_entropy(
                logits, targets
            )  # cross entropy expects the second argument to be the number of classes
    ...
    def generate(self, indx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            indx_cond = indx[
                :, -block_size:
            ]  # since the positional embeddings are only upto block_size
            # get the predictions
...
)  # about 10 M parameters
# 1M characters in our dataset which would be around 300,000 tokens in terms of OpenAI vocabulary
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0:
        losses = (
            estimate_loss()
        )  # because in previous method every batch can be more or less lucky
        print(
